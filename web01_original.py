...
def create_tables():
    conn = connect_db()
    cur = conn.cursor()
    cur.execute('''CREATE TABLE IF NOT EXISTS user(id INTEGER PRIMARY KEY AUTOINCREMENT, username VARCHAR(32), password VARCHAR(32))''')
    cur.execute('''CREATE TABLE IF NOT EXISTS time_line(id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, content TEXT, FOREIGN KEY (`user_id`) REFERENCES `user`(`id`))''')
    conn.commit()
    conn.close()
...
def init_data():
    conn = connect_db()
    cur = conn.cursor()
    cur.executemany('INSERT INTO `user` VALUES(NULL,?,?)', [('user1','123456'),('user2','123456')])
    cur.executemany('INSERT INTO `time_line` VALUES(NULL,?,?)', [(1,'Hello'),(1,'World'),(2,'Im 2'),(2,'Hello 2')])
    conn.commit()
    conn.close()
...
def render_login_page():
    return '''<form method="POST" style="margin:60px auto;width:200px;"><p><input name="username" type="text"/></p><p><input name="password" type="password"/></p><p><input value="Login" type="submit"/></p></form>'''
...
<div style="width:400px;margin:80px auto;">
    ...
    <form method="POST" action="/create_time_line">Add: <input type="text" name="content"/><input type="submit" value="Submit"/></form>
    <ul>{% for line in time_lines %}<li><p>{{ line['content'] }}</p>{% if line['user_id'] == user['id'] %}<a href="/delete/time_line/{{ line['id'] }}">Delete</a>{% endif %}</li>{% endfor %}</ul>
